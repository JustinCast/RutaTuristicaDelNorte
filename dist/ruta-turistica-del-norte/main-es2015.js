...
let AddServiceComponent = class AddServiceComponent {
    constructor(_fb, _dialog, _tour, _common) {
        this._fb = _fb;
     ...
     */
    addRates() {
        this._dialog.openRatesDialog().subscribe((rates) => {
            this.rates = rates;
...
let ShowImagesComponent = class ShowImagesComponent {
    constructor(data, fireStorage, snackbar, dialogRef, _common, _images) {
        this.data = data;
        ...
        this.fireStorage.storage
            ...
            this.data.table === "service"
                ? this._images
                    ...
                    .subscribe({
                    next: () => {
                        this._common.openSnackBar("Imagen eliminada de la base de datos", "Ok", 2500);
...
let AddTourComponent = class AddTourComponent {
    constructor(_fb, _dialog, _tour) {
        this._fb = _fb;
        this._dialog = _dialog;
        this._tour = _tour;
        this.phones = { phones: [] };
...
let AdminRootComponent = class AdminRootComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let DashboardComponent = class DashboardComponent {
    constructor(_user, _dialog, _router) {
        this._user = _user;
        this._dialog = _dialog;
        this._router = _router;
    }
    ngOnInit() {
        this._user.getServicesByUser().subscribe(data => {
            this.services = data;
            this._user.getToursByUser().subscribe({
                next: data => { this.tours = data; },
                error: (err) => this._user.handleError(err)
            });
        }, (err) => this._user.handleError(err));
    }
    editService(i) {
...
let EditServiceComponent = class EditServiceComponent {
    constructor(_active, _service, _common, _dialog) {
    ...
    ngOnInit() {
        ...
        if (id_service)
            this._service.getService(Number(id_service)).subscribe(service => {
                this.s = service;
                console.log(this.s);
                this.getServiceImages();
            }, (err) => this._service.handleError(err));
    }
    addPhone(phone) {
    ...
    openImagesDialog() {
        this._dialog.openImagesDialog(this.serviceImages.map(i => (i = i.url)), "service");
    }
    editLocation() {
        this._dialog.openPickLocationDialog(this.s._location).subscribe({
            ...
            error: err => this._service.openSnackBar(`Ha ocurrido un error: ${err}`, "Ok", 2500)
        });
    }
    onSubmit() {
...
let EditTourComponent = class EditTourComponent {
    constructor(_active, _tour, _service, _dialog) {
        this._active = _active;
        this._tour = _tour;
        this._service = _service;
        this._dialog = _dialog;
    ...
    ngOnInit() {
        ...
        if (id_tour)
            this._tour.getTour(Number(id_tour)).subscribe(tour => {
                this.t = tour;
                this.getToursNames();
            }, (err) => this._tour.handleError(err));
    }
    configFormControl() {
        this.filteredServices = this.serviceCtrl.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["startWith"])(""), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"])(value => this._filterTours(value)));
    }
    _filterTours(value) {
        const filterValue = value.toLowerCase();
        return this.servicesNames.filter(service => service.name.toLowerCase().includes(filterValue));
    }
    getTourImages() {
...
let LoginDialogComponent = class LoginDialogComponent {
    constructor(dialogRef, _fb, _user, _auth) {
        this.dialogRef = dialogRef;
        this._fb = _fb;
        this._user = _user;
        this._auth = _auth;
    ...
    onSubmit() {
        this._user
            ...
            next: data => {
                ...
                this._auth.logout();
                this._auth.login(new src_app_models_User__WEBPACK_IMPORTED_MODULE_6__["User"](data._fullname, data.logged, "", data._id));
                this._user.openSnackBar("Inicio de sesión exitoso", "Ok", 2500);
            },
            error: (err) => this._user.openSnackBar("Error de inicio de sesión", "Ok", 2500)
        });
    }
    passwordRecovery() {
...
let PickLocationComponent = class PickLocationComponent {
    constructor(dialogRef, location) {
        this.dialogRef = dialogRef;
        this.location = location;
    }
    ngOnInit() {
...
let SearchRelatedComponent = class SearchRelatedComponent {
...
let GeneralRootComponent = class GeneralRootComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let PasswordRecoveryComponent = class PasswordRecoveryComponent {
    ...
    onSubmit() {
        ...
        this._user
            ...
            .subscribe({
            next: r => this._user.openSnackBar(r === false
...
let SeeLocationComponent = class SeeLocationComponent {
...
let ServiceComponent = class ServiceComponent {
    constructor(_service, route, _router, _dialog, _tour) {
        this._service = _service;
        this.route = route;
        this._router = _router;
        this._dialog = _dialog;
    ...
    ngOnInit() {
        ...
        if (this.route.snapshot.paramMap.get("id_service") &&
            ...
            this.service = this._service.services.find(t => t.id_service === Number(id_service));
    ...
    ngAfterViewInit() {
        ...
        let e = document.getElementsByTagName("owl-carousel-child")[0];
        if (e) {
    ...
    handleGetRelatedToursSubscription(id_service) {
        this._tour.getRelatedTours(id_service).subscribe(related_tours => {
...
let TouristicOfferComponent = class TouristicOfferComponent {
    constructor(_service, _common, _tour, _dialog) {
        this._service = _service;
        this._common = _common;
        this._tour = _tour;
        this._dialog = _dialog;
    ...
    getTableCountData(columm, value) {
...
let VedaComponent = class VedaComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
class Service {
    constructor(_location, _name, _classification, _additional_info, _email, _website, _phones, imgs, _rates, id_service, _count, _imgs_lazy) {
...
class Tour {
    constructor(name, description, phones, imgs, email, id_user, related_service, _imgs_lazy) {
        this.name = name;
...
class User {
    constructor(fullname, username, password, id) {
...
let DialogManagerService = class DialogManagerService {
    ...
    openImagesDialog(images, tablename) {
        let dialogRef;
    ...
    showTourLocation(location, edit) {
        this.dialog
...
let Services = class Services {
    constructor(_http, _snackbar, _common) {
        this._http = _http;
        this._snackbar = _snackbar;
        this._common = _common;
    }
    getServicesCount(column, value) {
    ...
    getServices(limit, offset, filter) {
        this._http
    ...
    getServiceNameId(name) {
        let user_id = JSON.parse(localStorage.getItem(`${src_environments_environment__WEBPACK_IMPORTED_MODULE_3__["environment"].localstorage_key}`)).id;
...
let TourService = class TourService {
    ...
    saveTour(tour) {
        this._http
            ...
            .subscribe(() => this.openSnackBar("Tour guardado con éxito", "Ok", 2500), (err) => this.handleError(err));
    }
    getTour(id_tour) {
     ...
     */
    getRelatedTours(related_service) {
...
let UserService = class UserService {
    constructor(_http, _snackbar, _auth) {
        this._http = _http;
        this._snackbar = _snackbar;
        this._auth = _auth;
    }
    ...
    getServicesByUser() {
        let id = JSON.parse(localStorage.getItem(`${src_environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].localstorage_key}`)).id;
    ...
    verifyCodeUsername(username, code, newPassword) {
